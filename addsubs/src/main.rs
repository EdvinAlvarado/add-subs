...
fn addsubs<P: AsRef<Path>>(dir: P, videoformat: &str, subformat: &str, lang: &str) -> Result<Vec<Vec<u8>>, Box<dyn Error>> {
...
fn main() {
    ...
    let res = addsubs(&args[1], &args[2], &args[3], &args[4]).unwrap();
    for buf in res {
